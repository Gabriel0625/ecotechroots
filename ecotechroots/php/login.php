...
if (!$email || !$senha) {
    ...
    header('Location: ../telalogin.html'); exit;
}
if ($stmt = $mysqli->prepare('SELECT id_usuarios, senha, nm_usuario FROM tb_usuarios WHERE email = ? LIMIT 1')) {
    ...
    if ($user = $res->fetch_assoc()) {
        ...
        } else { $_SESSION['login_error'] = 'Senha incorreta.'; }
    } else { $_SESSION['login_error'] = 'Usuário não encontrado.'; }
    $stmt->close();
} else { $_SESSION['login_error'] = 'Erro no sistema.'; }
header('Location: ../telalogin.html'); exit;
?>